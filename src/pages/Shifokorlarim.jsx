  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-7 gap-1 text-center text-[10px] font-semibold mb-2 text-gray-400">
        {days.map((day) => (
          <div key={day} className="h-4 flex items-center justify-center">{day}</div>
        ))}
      </div>
          ...
          return (
            <div
              ...
              className={`
                ...
                ${isToday ? `bg-[${THEME_COLOR}] text-white font-bold shadow-md` : isPast ? 'text-gray-300' : 'text-gray-700'}
              `}
            >
              {day}
            </div>
...
// === FullCalendar used inside modal ===
const FullCalendar = ({ selectedDate, setSelectedDate }) => {
  ...
  const goToNextMonth = () => setDate(prev => new Date(prev.getFullYear(), prev.getMonth() + 1, 1));
  const goToPrevMonth = () => setDate(prev => new Date(prev.getFullYear(), prev.getMonth() - 1, 1));
  const goToYear = (year) => setDate(prev => new Date(year, prev.getMonth(), 1));

  return (
    <div className={`p-6 rounded-2xl bg-white text-[${DARK_TEXT}] shadow-2xl w-full max-w-sm`}>
      <div className="flex justify-between items-center mb-5">
        <button onClick={goToPrevMonth} className={`p-2 rounded-full hover:bg-gray-100 transition text-[${THEME_COLOR}]`}><FiChevronLeft className="text-xl" /></button>
        <div className="text-lg font-bold flex items-center gap-2">
          <span className={`cursor-pointer hover:text-[${THEME_COLOR}]`} onClick={() => console.log('Yil tanlash modalini ochish')}>{date.getFullYear()}</span>
          <span className={`text-[${THEME_COLOR}]`}>{months[date.getMonth()]}</span>
        </div>
        <button onClick={goToNextMonth} className={`p-2 rounded-full hover:bg-gray-100 transition text-[${THEME_COLOR}]`}><FiChevronRight className="text-xl" /></button>
      </div>
          ...
          return (
            <div
              ...
              className={`
                ...
                ${isToday ? 'border-2 border-red-500 text-gray-700 font-bold' : isSelected ? `bg-[${THEME_COLOR}] text-white font-bold shadow-md cursor-pointer` : isPast ? 'text-gray-300' : 'text-gray-700 hover:bg-gray-100 cursor-pointer'}
              `}
            >
              {dayNumber}
            </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="relative" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className={`absolute top-[-10px] right-[-10px] bg-white text-[${THEME_COLOR}] p-2 rounded-full shadow-lg border-2 border-gray-100 hover:bg-gray-100 transition z-10`}>
  ...
  if (file) {
    return (
      <div className="relative w-full aspect-square rounded-xl overflow-hidden shadow-md">
        {file.type === 'image' ? (
          <img src={file.preview} alt="Yuklangan rasm" className="w-full h-full object-cover" />
        ) : (
          <video src={file.preview} className="w-full h-full object-cover" controls loop muted />
        )}
        <button onClick={handleRemoveFile} className="absolute top-2 right-2 p-1.5 bg-black/50 hover:bg-black/70 rounded-full text-white transition z-20">
  ...
  if (isPlaceholder) {
    return (
      <div className={`relative w-full aspect-square bg-gray-100 rounded-xl overflow-hidden flex flex-col items-center justify-center border-2 border-dashed ${type === 'image' ? `border-[${THEME_COLOR}]/50` : 'border-blue-400/50'} transition hover:bg-gray-200 cursor-pointer`}>
        <input type="file" accept={type === 'image' ? "image/*" : "video/*"} className="absolute inset-0 opacity-0 cursor-pointer z-10" onChange={handleFileChange} />
        {type === 'image' ? <FaPlus className={`text-xl mb-1 text-[${THEME_COLOR}]`} /> : <FiVideo className="text-xl mb-1 text-blue-500" />}
        <p className={`text-center text-xs font-medium ${type === 'image' ? `text-[${THEME_COLOR}]` : 'text-blue-500'}`}>{type === 'image' ? "Rasm joylash" : "Video joylash"}</p>
      </div>
    );
  }
...
function Shifokorlarim() {
  ...
  const allDoctors = [
    { id: 1, img: Denta5, name: "Dr. Omontayev Xayotillo", job: "Terapet", rating: "5.0", patients: "200 ta bemor", exp: "3+ yil", price: "350 000 so’m", location: "Namangan vil., Davlatobod t., 5-kichik nohiya, 34.", clinic: "Markaziy Poliklinika", is247: false },
  ...
  const currentDoctor = useMemo(() => {
    ...
    return allDoctors.find(doc => doc.id === doctorId);
  }, [id]);
  ...
  if (!currentDoctor) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-gray-100 text-red-600">
        ...
        <Link to="/boshsaxifa" className={`bg-[${THEME_COLOR}] text-white py-2 px-4 rounded-full font-medium hover:opacity-90`}>Bosh sahifaga qaytish</Link>
      </div>
    );
  }
  ...
  useEffect(() => {
    try {
      ...
      const found = fav.some(d => d.id === currentDoctor.id);
      setIsLiked(found);
    } catch (e) {
      console.error('favoriteDoctors parse error', e);
      setIsLiked(false);
    }
  }, [currentDoctor]);
  ...
  const handleToggleLike = () => {
    try {
      const fav = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]');
      const exists = fav.some(d => d.id === currentDoctor.id);

      if (exists) {
        const updated = fav.filter(d => d.id !== currentDoctor.id);
        ...
        const toStore = {
          ...
          job: currentDoctor.job,
          img: currentDoctor.img,
          rating: currentDoctor.rating,
          patients: currentDoctor.patients,
          exp: currentDoctor.exp,
          price: currentDoctor.price,
          is247: currentDoctor.is247,
  ...
  const handleSetImageFile = (index, file) => {
    setImageFiles(prevFiles => {
      ...
      if (file === null) newFiles.splice(index, 1);
      else if (index === newFiles.length) newFiles.push(file);
      else newFiles[index] = file;
      return newFiles;
    });
  };
  ...
  const imageGallery = useMemo(() => {
    ...
    if (gallery.length < 5) gallery.push(null);
    return gallery;
  }, [imageFiles]);
  ...
  return (
    <div className="w-full mx-auto mb-[150px] bg-white min-h-screen pb-10 relative">
      <CalendarModal isOpen={isCalendarModalOpen} onClose={() => setIsCalendarModalOpen(false)} selectedDate={selectedDate} setSelectedDate={setSelectedDate} />
      ...
      <div className={`relative z-10 bg-[${THEME_COLOR}] text-white p-5 pb-10 md:p-10 md:pb-14 lg:px-[70px] lg:pt-[40px] lg:pb-16 xl:px-[100px] xl:pt-[50px] xl:pb-20 rounded-b-[35px] shadow-xl`}>
        <div className="flex justify-between items-start w-full">
          <Link to={"/boshsaxifa"}>
            <div className="text-3xl md:text-4xl"><HiOutlineArrowLeft /></div>
          </Link>

          <div className="text-3xl md:text-4xl cursor-pointer transition" onClick={handleToggleLike} role="button">
            {isLiked ? <AiFillHeart className="text-red-500 transition" /> : <AiOutlineHeart className="text-white hover:text-red-300 transition" />}
          </div>
        </div>
        ...
        <div className="flex items-start justify-between gap-10 mt-5 md:mt-8">
          <div className="flex items-center gap-4 md:gap-6 lg:gap-8 max-w-[calc(100%-240px)] lg:max-w-none">
            <img src={currentDoctor.img} className="w-28 h-20 md:w-36 md:h-28 lg:w-44 lg:h-32 rounded-xl border-[3.5px] border-white object-cover" alt={currentDoctor.name} />
            <div>
              <h2 className="text-xl md:text-2xl font-semibold">{currentDoctor.name}</h2>
              <div className="flex items-center gap-2 mt-1">
                ...
                <div className="flex items-center gap-1 bg-white/30 px-2 rounded-full">
                  ...
                  <span className="text-[12px] md:text-sm font-medium">{currentDoctor.rating}</span>
                </div>
              </div>
              <p className="text-sm md:text-base text-white/80 mt-1">{currentDoctor.clinic}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`translate-y-[-10px] m-auto bg-white text-gray-700 w-[86%] shadow-xl rounded-b-2xl border-t-2 border-[${THEME_COLOR}]/50 px-6 py-6 md:px-8 md:py-7 flex justify-between`}>
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <span className="text-sm md:text-base text-gray-600">{currentDoctor.patients}</span>
        </div>

        <div className="flex items-center gap-2 md:gap-3">
          <MdWorkOutline className={`text-xl md:text-2xl text-[${THEME_COLOR}]`} />
          <span className="text-sm md:text-base text-gray-600">{currentDoctor.exp} tajriba</span>
        </div>
      </div>
      ...
      <div className="xl:px-[100px] lg:px-[70px] md:p-10 px-5 md:px-8 mt-1.5 space-y-4">
        <div className="flex items-start gap-3">
          ...
          <p className="text-sm md:text-base text-gray-700 leading-6">{currentDoctor.location}</p>
        </div>

        <div className="flex items-start gap-3">
          <BsClockHistory className={`text-xl md:text-2xl text-[${THEME_COLOR}]`} />
          <p className="text-sm md:text-base text-gray-700 leading-6">{currentDoctor.is247 ? "24/7 ochiq." : "9:00 dan 18:00 gacha ochiq."}</p>
        </div>

        <div className="flex items-start gap-3">
          <MdOutlineAttachMoney className={`text-xl md:text-2xl text-[${THEME_COLOR}]`} />
          <p className="text-sm md:text-base text-gray-700 leading-6">{currentDoctor.price} o‘rtacha narx</p>
        </div>
      </div>

      <div className="xl:px-[100px] lg:px-[70px] md:p-10 px-5 flex items-center gap-3 mt-6">
        <button onClick={() => setActiveTab('work')} className={`flex-1 w-[200px] py-2 md:py-3 rounded-full font-medium text-sm md:text-base transition ${activeTab === 'work' ? `bg-[${THEME_COLOR}] text-white shadow-md` : 'bg-gray-200 text-gray-600 hover:bg-gray-300'}`}>Olingan ishlar</button>
        <button onClick={() => setActiveTab('reviews')} className={`flex-1 w-[200px] py-2 md:py-3 rounded-full font-medium text-sm md:text-base transition ${activeTab === 'reviews' ? `bg-[${THEME_COLOR}] text-white shadow-md` : 'bg-gray-200 text-gray-600 hover:bg-gray-300'}`}>Sharhlar</button>
      </div>

      {activeTab === 'work' && (
        <div className="px-5 md:px-8 lg:px-[70px] xl:px-[100px] mt-6">
          <h2 className={`text-lg md:text-xl font-semibold mb-4 text-[${DARK_TEXT}]`}>Foto galereya</h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {imageGallery.map((file, index) => (
              <UploaderPlaceholder key={index} type="image" file={file} isPlaceholder={file === null} setFile={(newFile) => handleSetImageFile(index, newFile)} />
            ))}
          </div>

          <h2 className={`text-lg md:text-xl font-semibold mt-8 mb-4 text-[${DARK_TEXT}]`}>Video galereya</h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {videoGallery.map((file, index) => (
              <UploaderPlaceholder key={index} type="video" file={file} isPlaceholder={file === null} setFile={(newFile) => handleSetVideoFile(index, newFile)} />
            ))}
          </div>
        </div>
      )}
          ...
          {reviewsData.map((review) => (
            <div key={review.id} className="bg-gray-50 p-4 rounded-xl shadow-sm border border-gray-100">
              <div className="flex items-start space-x-3 mb-2">
                <img src={review.image} alt={review.name} className="w-10 h-10 rounded-full object-cover flex-shrink-0" />
              ...
              <p className={`text-sm text-gray-700 mt-2 leading-relaxed italic border-l-2 border-[${THEME_COLOR}] pl-2`}>"{review.text}"</p>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="px-5 md:px-8 lg:px-10 py-3 w-full mt-5 bg-white fixed bottom-[70px] left-0 flex items-center justify-between shadow-2xl border-t border-gray-100">
        <Link to={"/qabulgayozilish"} state={{ doctor: currentDoctor }} className="flex-1">
          <button className={`w-full bg-[${THEME_COLOR}] text-white py-3 rounded-2xl font-medium text-[15px] md:text-base transition hover:opacity-90 shadow-md`}>Qabulga yozilish</button>
        </Link>
