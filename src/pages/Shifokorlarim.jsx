...
import { useParams, Link } from "react-router-dom"; // useParams ni import qilish
...
import Denta2 from "../assets/denta2.jpg"; // Yangi importlar
import Denta3 from "../assets/denta3.jpg"; // Yangi importlar
import Denta4 from "../assets/denta4.jpg"; // Yangi importlar
import Denta5 from "../assets/denta5.png"; // Yangi importlar
...
// ... (renderStars, StylishCalendar, FullCalendar, CalendarModal, UploaderPlaceholder komponentlari o'zgarishsiz qoldirildi) ...
...
                        onClick={() => console.log('Yil tanlash modalini ochish')} 
                    >
                        {date.getFullYear()}
                    </span>
...
function Shifokorlarim() {
    // 1. useParams hooki yordamida ID ni olish
    const { id } = useParams();

    // 2. Doctorlar ma'lumotlari (Boshsaxifa bilan sinxron)
    const allDoctors = [
        ...
        // ... (boshqa doctorlar ham shu yerda bo'lishi kerak)
    ];

    // 3. ID bo'yicha doctorni topish
    const currentDoctor = useMemo(() => {
        // useParams orqali olingan ID string bo'ladi, uni Number ga o'girish muhim
        const doctorId = parseInt(id); 
        return allDoctors.find(doc => doc.id === doctorId);
    }, [id]);


    // 4. Agar doctor topilmasa, xatolik ko'rsatish
    if (!currentDoctor) {
        return (
    ...
    // Doctor ma'lumotlari topildi, endi uni ishlatamiz
...
    // Rasm/video funksiyalari (o'zgarishsiz)
...
    // Sharhlar uchun ma'lumotlar (dinamik bo'lmagani uchun statik qoldirildi)
    const reviewsData = [
...
    ];
    
    // Yurakcha bosilganda holatni o'zgartirish funksiyasi
...
      {/* HEADER - MA'LUMOTLAR DINAMIK QILINDI */}
      <div
        className={`
...
                {currentDoctor.name} {/* DINAMIK */}
              </h2>

              <div className="flex items-center gap-2 mt-1">
                <p className="text-sm md:text-base text-white/90">{currentDoctor.job}</p> {/* DINAMIK */}
                <div className="flex items-center gap-1 bg-white/30 px-2 rounded-full">
                  <FaStar className="text-yellow-300" />
                  <span className="text-[12px] md:text-sm font-medium">{currentDoctor.rating}</span> {/* DINAMIK */}
                </div>
              </div>

              <p className="text-sm md:text-base text-white/80 mt-1">
                {currentDoctor.clinic} {/* DINAMIK */}
              </p>
            </div>
          </div>
...
      {/* Patients / Experience Box - MA'LUMOTLAR DINAMIK QILINDI */}
      <div
        className={`
          translate-y-[-10px]
...
          <span className="text-sm md:text-base text-gray-600">{currentDoctor.patients}</span> {/* DINAMIK */}
          
        </div>

        <div className="flex items-center gap-2 md:gap-3">
          <MdWorkOutline className={`text-xl md:text-2xl text-[${THEME_COLOR}]`} />
          <span className="text-sm md:text-base text-gray-600">{currentDoctor.exp} tajriba</span> {/* DINAMIK */}
        </div>
      </div>

      {/* INFO LIST - MA'LUMOTLAR DINAMIK QILINDI */}
...
            {currentDoctor.location} {/* DINAMIK */}
          </p>
          
        </div>
...
          <p className="text-sm md:text-base text-gray-700 leading-6">
            {currentDoctor.is247 ? "24/7 ochiq." : "9:00 dan 18:00 gacha ochiq."} {/* DINAMIK */}
          </p>
        </div>
...
          <p className="text-sm md:text-base text-gray-700 leading-6">
            {currentDoctor.price} o‘rtacha narx {/* DINAMIK */}
          </p>
        </div>
      </div>
...
      {/* BOTTOM BAR (Qabulga yozilish tugmasi endi Link bilan o'ralgan) */}
      <div
        className="
...
      >
        
        {/* Qabulga yozilish - /qabulgayozilish manziliga ulanadi */}
