...
navigate("/Notification")
  ...
  // --- YANGI QISM: Doctorlar ma'lumotlarini massiv ko'rinishida saqlash ---
  const doctors = [
...
      {/* Filterlar – YANGI VERSIYA (O'zgarmadi) */}
...
            className={`px-4 py-2 rounded-2xl flex items-center gap-2 text-[12px] sm:text-[14px] font-medium cursor-pointer transition-all bg-[#BDF3FF]`}
          >
...
            className={`px-4 py-2 rounded-2xl flex items-center gap-2 text-[12px] sm:text-[14px] font-medium cursor-pointer transition-all bg-[#E5E7EB]`}
          >
...
      {/* Qolgan qismi (DoctorCard lar) - ASOSIY O'ZGARTIRILADIGAN QISM */}
      <div className="md:px-10 lg:px-[70px] px-2 mb-5 xl:px-[100px]">
        <h1 className="text-2xl md:text-3xl font-semibold mb-4">
          Hozirda mavjud mutaxassislar
        </h1>
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4">
          {/* Oldingi DoctorCard chaqiruvlari o'rniga, endi doctors massivi map qilinadi */}
            {doctors.map((doctor) => (
                <DoctorCard 
                    key={doctor.id} // Reactga kerakli key
                    id={doctor.id} // DoctorCardga uzatilayotgan noyob ID
                    img={doctor.img} 
                    name={doctor.name} 
                    job={doctor.job} 
