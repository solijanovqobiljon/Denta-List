...
import { FaStar } from "react-icons/fa"; // Yulduz ikonkasini qo'shdim

// Rang konstantalari (Tailwind bilan ishlash uchun)
const FEMALE_ACCENT = '#F269CD'; // Asosiy pushti
const DARK_TEXT = '#272937'; 
const GREEN_BADGE = '#4cd964';

function CardAyollar({ img, name, job, rating, distance, price, patients, exp, service }) {
...
        <p className={`text-[${FEMALE_ACCENT}] text-sm font-medium`}>{job}</p> {/* Kasb nomi pushti */}

        <div className={`flex items-center gap-4 mt-2 text-xs font-semibold text-[${FEMALE_ACCENT}]/80`}> {/* Ma'lumotlar pushtiroq rangda */}
...
        <Link to={"/shifokorlar"} >
        <button 
            className={`w-full bg-[${FEMALE_ACCENT}] text-white py-2.5 rounded-full mt-3 text-base font-semibold transition hover:bg-pink-600 shadow-md shadow-pink-300`}
        >
          Profilni ko‘rish
        </button>
        </Link>
